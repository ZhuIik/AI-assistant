...
сохранение человекочитаемых транскриптов в data/transcripts

сохранение структурированных `.segments.jsonl` с таймкодами, текстом и спикерами в data/segments для дальнейшей разметки лекций

отдельная папка data/annotations зарезервирована под аннотации
...
4. Сборка черновиков аннотаций из сегментов
python scripts/build_annotation_drafts.py

5. Автозаполнение полей аннотаций
...
8. Запуск клиента (Telegram или другой)
python scripts/bot.py

Черновики аннотаций сохраняются в `data/annotations/*.draft.jsonl`.
При необходимости можно создать `data/annotations/speaker_map.json` по примеру `speaker_map.example.json`,
чтобы автоматически подставлять `professor|student` на этапе сборки.
Автозаполненные аннотации сохраняются в `data/annotations/*.annotations.jsonl`.
