...
После ответа обязательно добавь один конкретный ситуационный пример:
Пример

Пример нужно сгенерировать самому. Это должен быть конкретный мини-кейс или сценарий
с участниками, проектом, решением, расчетом или действием. Он должен иллюстрировать
именно найденный смысл, а не быть общим пересказом.
...
Сформируй ответ так:
Сначала дай подробный основной ответ без заголовка: 2-4 содержательных абзаца, объясняющих суть, логику и важные детали.
Затем добавь только один раздел "Пример". Пример должен отражать именно этот ответ и быть конкретной ситуацией, а не общим пересказом.
